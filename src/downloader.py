"""A dedicated class for downloading media items from URLs."""
...
class MediaDownloader:
    ...
    def __init__(self, session: requests.Session):
        """Initialize the MediaDownloader with an authenticated session.
    ...
    def download_media_for_story(
        self, story: dict[str, Any], download_base_path: str
    ) -> bool:
        """Download all media for a single story, creating a new subfolder for it.
        ...
        story_title = self._sanitize_story_title(
            story.get("title", f"story_{story_id}")
        )
        media_items = story.get("media", [])

        # Use updated_at or created_at for the date part of the folder name
        story_date_sanitized = self._sanitize_story_datetime(
            story.get("updated_at", story.get("created_at", ""))
        )

        if not media_items:
            _LOGGER.info(
                "Story '%s' (ID: %s) has no media to download.", story_title, story_id
            )
            return True

        _LOGGER.info(
            "Processing media for story '%s' (ID: %s)...", story_title, story_id
        )
        ...
        for item in media_items:
            ...
            if not file_url:
                _LOGGER.warning(
                    "Media item in story %s has no valid URL. Skipping.", story_id
                )
                continue
